...
def find_best_parameters():
    ...
    costheta2 = np.cos(theta_values)
    sintheta2 = np.sin(theta_values)
        ...
        for p01 in p_values:
            ...
            for theta in theta_values:
                ...
                if abs(lambda1 - problem.value) < tolerance:
                    ...
                    results.append((p00, p01, p10, p11, alpha, theta.item(), cosbeta2, lambda1, lambda2_value, problem.value, gamma.value))

    # 输出所有符合条件的结果（在遍历结束后）
    if results:
        print("找到的参数组合和对应的 gamma 矩阵：")
        for p00, p01, p10, p11, alpha, theta, cosbeta2, lambda1, lambda2, problem_value, gamma_matrix in results:
            print(f"p00={p00:.2f}, p01={p01:.2f}, p10={p10:.2f}, p11={p11:.2f}, "
                  f"alpha={alpha:.2f}, theta={theta:.2f}, cosbeta2={cosbeta2:.5f}, "
                  f"lambda1={lambda1:.5f}, lambda2={lambda2:.5f}, problem.value={problem_value:.5f}")
            print("gamma matrix:")
            print(gamma_matrix)
            print("\n")
    else:
