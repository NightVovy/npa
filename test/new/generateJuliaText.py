def generate_text(p, q, alpha, layer):
    ...
    text = f'npa_max({alpha:.5f} * A1 + {p00:.5f} * A1 * B1 + {p01:.5f} * A1 * B2 + {p10:.5f} * A2 * B1 - {p11:.5f} * A2 * B2, "{layer}")'
...
def generate_second_text(p00_input, p01_input, p10_input, p11_input, alpha, layer2):
    ...
    text = f'npa_max({alpha:.5f} * A1 + {p00_input} * A1 * B1 + {p01_input} * A1 * B2 + {p10_input} * A2 * B1 - {p11_input} * A2 * B2, "{layer2}")'
...
q = 0.1
alpha = 1.6
layer = "1 + A B + A^2 B"  # 如果需要修改 layer 的值
